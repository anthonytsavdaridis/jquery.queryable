/*! queryable v1.0 | https://github.com/anthonytsavdaridis/jquery.queryable | (c) 2013 */
...
(function(window)
{
	...
	var _iterator = function(source, callback)
	{
		...
		if(isArray)
		{
			for(;i < length; i++)
			{
				value = callback.call(source[i], i, source[i]);
				if(value === false)
					break;
			}
		}
	...
	for(var i in Queryable)
		OrderedQueryable[i] = Queryable[i];
